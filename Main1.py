from Expectimax1_v2 import Node as Node
...
with open(filename, "a+", encoding="utf-8", newline="") as f:
    ...
    for k in range(game_count):
        T = [8 * int(pow(4, 15)) for i in range(b)]
        score_list = []
        scn_list1 = []
        scn_list2 = []
        step = 0
        ...
        while not game.is_end(game.grid):
            ...
            node = Node(game.grid, 0, max_depth, "Max", T)
            ...
            step += 1
            game.random_field()
        terminal_node = Node(game.grid, 0, max_depth, "Max", T)
